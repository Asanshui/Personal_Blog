...
class Worker:
    ...
    def start_video_capture(self, camera_index=0):
        ...
        if not self.video_capture.isOpened():
            QMessageBox.warning(None, "警告", "无法打开摄像头")
            return False
        return True
    ...
    def process_video_frame(self):
        ...
        if ret:
            ...
            self.current_annotated_image = annotated_frame
            self.update_frame(frame, annotated_frame)

    def update_frame(self, original_frame, annotated_frame):
        ...
        self.main_window.label1.setPixmap(pixmap1.scaled(self.main_window.label1.size(), Qt.KeepAspectRatio))
        ...
        self.main_window.label2.setPixmap(pixmap2.scaled(self.main_window.label2.size(), Qt.KeepAspectRatio))
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.camera_detect_button.clicked.connect(self.start_camera)
    ...
    def select_video(self):
        ...
        if video_path:
            self.worker.start_video_capture(video_path)
            self.worker.timer.timeout.connect(self.worker.process_video_frame)
            self.worker.timer.start(30)  # 30ms per frame

    def start_camera(self):
        if self.worker.start_video_capture(0):  # 0 is the default camera index
            self.worker.timer.timeout.connect(self.worker.process_video_frame)
            self.worker.timer.start(30)  # 30ms per frame
